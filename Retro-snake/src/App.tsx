...
export default function App() {
  ...
  const getRandomPosition = useCallback((currentSnake: Position[], currentObstacles: Obstacle[]): Position => {
    ...
    while (collision) {
      ...
      const hitObstacle = currentObstacles.some((o) => o.x === newPos.x && o.y === newPos.y);
  ...
  // Direction Change Handler (Prevent 180-degree instant suicide)
      ...
      setSnake((prevSnake) => {
        ...
        if (food && head.x === food.position.x && head.y === food.position.y) {
          ...
          if (food.type === 'golden') {
            ...
            sound.playSpecialEat();
            setIsIceActive(true);
            setTimeout(() => setIsIceActive(false), 5000); // 5 sec slow-mo
          } else {
            sound.playEat();
          }
